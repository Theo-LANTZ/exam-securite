...
if(isset($_POST['formredaction']))
	...
	if(!empty($_POST['titre']) AND !empty($_POST['textnews']))
		{		
			...
			if($titrelength <= 255)
			{	
			$insertnews = $bdd -> prepare("INSERT INTO news(idnews, idtheme, titrenews, datenews, textnews, idredacteur) VALUES(NULL, ?, ?, ?, ?, ?)")
			;
			$insertnews->execute(array($idtheme, $titre, $date, $textnews, $_SESSION['idredacteur']));
